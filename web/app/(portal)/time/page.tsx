...
import { currentTimeIn, formatDate, formatDateTime, formatTime } from "@/lib/timezone";
...
export default async function TimePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="card overflow-x-auto"><div className="panel-heading"><div><span className="panel-icon"><Icon name="reports" /></span><div><h3>Attendance history</h3><p>Your newest records appear first.</p></div></div></div><table className="w-full text-sm"><thead><tr className="border-b border-stone-100 text-left"><th className="pb-3">Date</th><th className="pb-3">Clock in</th><th className="pb-3">Clock out</th><th className="pb-3">Duration</th><th className="pb-3">Status</th><th className="pb-3 text-right">Action</th></tr></thead><tbody className="divide-y divide-stone-100">{(sessions ?? []).length === 0 && <tr><td colSpan={6}><div className="context-empty table-context-empty"><span><Icon name="clock" /></span><div><strong>No attendance records yet</strong><p>Your first completed shift will appear here with its start, end, and duration.</p></div></div></td></tr>}{(sessions ?? []).map((item) => <tr key={item.id}><td className="py-3 font-medium text-stone-900">{formatDate(item.clock_in_at, session.organization?.timezone)}</td><td className="py-3">{formatTime(item.clock_in_at, session.organization?.timezone)}</td><td className="py-3">{item.clock_out_at ? formatTime(item.clock_out_at, session.organization?.timezone) : "—"}</td><td className="py-3">{duration(item.clock_in_at, item.clock_out_at)}</td><td className="py-3"><span className={`badge ${statusBadgeClass(item.status)}`}>{item.status.replace(/_/g," ")}</span></td><td className="py-3 text-right">{item.clock_out_at && <AttendanceCorrectionButton sessionId={item.id} clockInAt={item.clock_in_at} clockOutAt={item.clock_out_at} />}</td></tr>)}</tbody></table></section>
      {(adjustments ?? []).length > 0 && <section className="card"><div className="panel-heading"><div><span className="panel-icon"><Icon name="check" /></span><div><h3>Correction requests</h3><p>Decisions stay visible alongside the audit trail.</p></div></div></div><div className="correction-list">{(adjustments ?? []).map((item) => <div key={item.id}><div><strong>{item.field === "clock_in_at" ? "Clock-in" : "Clock-out"} correction</strong><small>Requested {formatDate(item.requested_at, session.organization?.timezone)} · {formatDateTime(item.requested_value, session.organization?.timezone)}</small></div><span className={`badge ${statusBadgeClass(item.status)}`}>{item.status}</span></div>)}</div></section>}
    </div>
  );
}
